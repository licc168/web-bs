...
public final class PluginAggregator implements Plugin {
    ...
    @Override
    public boolean sqlMapPageElementGenerated(XmlElement element, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.sqlMapPageElementGenerated(element, introspectedTable)) {
                rc = false;
                break;
            }
        }
    ...
    @Override
    public boolean clientPageMethodGenerated(Method method, Interface interfaze, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.clientPageMethodGenerated(method,
